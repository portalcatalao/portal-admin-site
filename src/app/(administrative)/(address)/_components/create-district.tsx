...
export function ModalCreateDistrict({city}: {city: ICity}) {
    ...
    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            setLoading(true);
            const {success, message} = await api.post('address/districts', {
                name: name.value,
                city: {
                    id: city.id
                }
            ...
            if(success) {
                notification.execute('success', 'Bairro cadastrado com sucesso.');
                router.refresh();
                setShow(false);
            } else {
    ...
    return (
        <>
            <button className={components.button.primary + ' text-sm'} onClick={() => setShow(true)}>Cadastrar bairro</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Cadastrar bairro em {city.name}</h4>
                            </div>

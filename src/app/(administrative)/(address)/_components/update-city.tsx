...
export function ModalUpdateCity({ city }: { city: ICity }) {
    const router = useRouter();
    const notification = useNotification();
    const [show, setShow] = useState(false);
    const [loading, setLoading] = useState(false);
    const name = useForm('text', city?.name);

    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            setLoading(true);
            const { success, message } = await api.patch(`address/cities/${city.id}`, {
                name: name.value
            }).then(res => res.data);
            if (success) {
    ...
    return (
        <>
            <button className={components.button.secondary + ' h-12'} onClick={() => setShow(true)}>Editar cidade</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Editar cidade</h4>
                            </div>

...
export function ModalUpdateDistrict({district}: {district: IDistrict}) {
    const router = useRouter();
    const notification = useNotification();
    const [show, setShow] = useState(false);
    const [loading, setLoading] = useState(false);
    const name = useForm('text', district?.name);

    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            setLoading(true);
            const {success, message} = await api.patch(`address/districts/${district.id}`, {
                name: name.value
            }).then(res => res.data);
            if(success) {
                notification.execute('success', 'Bairro atualizado com sucesso.');
                router.refresh();
                setShow(false);
            } else {
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Editar bairro</h4>
                            </div>

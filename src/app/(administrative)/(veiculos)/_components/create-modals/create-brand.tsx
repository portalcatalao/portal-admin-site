...
export function ModalCreateBrand() {
    ...
    const handleSubmit = async (e) => {
        try {
            e.preventDefault();
            setLoading(true);
            const {success, message} = await apiAdmin.post('brands', {
                name: name.value,
                type: 'brandcar'
            }).then(res => res.data);
            if(success) {
                notification.execute('success', 'Marca cadastrada com sucesso.');
                router.refresh();
                setShow(false);
            } else {
    ...
    return (
        <>
            <button className={components.button.primary + ' text-sm'} onClick={() => setShow(true)}>Cadastrar marca</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Cadastrar marca em carros</h4>
                            </div>

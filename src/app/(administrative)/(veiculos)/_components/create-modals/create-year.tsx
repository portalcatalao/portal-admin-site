...
export function ModalCreateYear({ version }: {version: IVersionModel}) {
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            if (success) {
                notification.execute('success', 'Ano da versão cadastrado com sucesso.');
                router.refresh();
                setShow(false);
            } else {
    ...
    return (
        <>
            <button className={components.button.primary + ' text-sm'} onClick={() => setShow(true)}>Cadastrar ano</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Cadastrar ano em {version.name}</h4>
                            </div>

...
export function ModalUpdateVersion({version}: {version: IVersionModel}) {
    const router = useRouter();
    const notification = useNotification();
    const [show, setShow] = useState(false);
    const [loading, setLoading] = useState(false);
    const name = useForm('text', version?.name);
    const fipe_code = useForm('text', version?.fipe_code);
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            if(success) {
                notification.execute('success', 'Versão atualizada com sucesso.');
                router.refresh();
                setShow(false);
            } else {
    ...
    return (
        <>
            <button className={components.button.secondary + ' h-12 text-sm'} onClick={() => setShow(true)}>Editar versão</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Atualizar versão</h4>
                            </div>

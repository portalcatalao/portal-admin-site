...
export function ModalUpdateModel({model}: {model: IModel}) {
    const router = useRouter();
    const notification = useNotification();
    const [show, setShow] = useState(false);
    const [loading, setLoading] = useState(false);
    const name = useForm('text', model?.name);
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const {success, message} = await apiAdmin.patch(`models/${model.id}`, newModel).then(res => res.data);
            if(success) {
                notification.execute('success', 'Modelo atualizado com sucesso.');
                router.refresh();
                setShow(false);
            } else {
    ...
    return (
        <>
            <button className={components.button.secondary + ' h-12 text-sm'} onClick={() => setShow(true)}>Editar modelo</button>
            {show &&
                    ...
                    <div className="w-full max-w-xl bg-white p-3 rounded-md relative">
                        <div className="flex gap-2">
                            <div className="flex-1">
                                <h4 className="font-semibold text-zinc-900 text-lg">Editar modelo</h4>
                            </div>

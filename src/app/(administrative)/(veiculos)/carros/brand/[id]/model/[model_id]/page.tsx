...
export default async function Page({ params }: { params: { id: string, model_id: string } }) {
    ...
    if(!result) return (
        <div className="flex flex-col">
            <Link href={`/carros/brand/${params.id}`} className={components.button.secondary + ' w-fit'}><FiChevronLeft /> Voltar</Link>
    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className="flex gap-4 lg:flex-row items-center justify-between">
                <div className="flex flex-col gap-4">
                    <div className="flex gap-2 items-center">
                        <Link href={`/carros/brand/${params.id}`} className={components.button.icon_small}><FiChevronLeft /></Link>
            ...
            <div className='grid md:grid-cols-4 gap-4'>
                {results?.map((item) =>
                    <Link href={`/carros/brand/${params.id}/model/${params.model_id}/version/${item.id}`} className="p-3 bg-white border rounded-md flex gap-3 justify-between items-center hover:shadow-sm capitalize text-sm">
                        {item.name}
                        <FiArrowRight />
                    </Link>
                )}

...
export default async function Page({ params }: { params: { id: string } }) {
    ...
    if(!result) return (
        <div className="flex flex-col">
            <Link href={`/motos`} className={components.button.secondary + ' w-fit'}><FiChevronLeft /> Voltar</Link>
    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className="flex gap-4 lg:flex-row items-center justify-between">
                <div className="flex flex-col gap-4">
                    <div className="flex gap-2 items-center">
                        <Link href={`/motos`} className={components.button.icon_small}><FiChevronLeft /></Link>
            ...
            <div className='grid md:grid-cols-4 gap-4'>
                {results?.map((item) =>
                    <Link href={`/motos/brand/${result.id}/model/${item.id}`} className="p-3 bg-white border rounded-md flex justify-between items-center hover:shadow-sm capitalize">
                        {item.name}
                        <FiArrowRight />
                    </Link>
                )}

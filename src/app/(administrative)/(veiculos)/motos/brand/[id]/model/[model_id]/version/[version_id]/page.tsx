...
export default async function Page({ params }: { params: { id: string, model_id: string, version_id: string  } }) {
    ...
    if(!result) return (
        <div className="flex gap-4 flex-col">
            ...
            <Link href={`/motos/brand/${params.id}/model/${params.model_id}`} className={components.button.secondary + ' w-fit'}><FiChevronLeft /> Voltar</Link>
        </div>
    )
            ...
            <div className="flex gap-4 lg:flex-row items-center justify-between">
                <div className="flex flex-col gap-4">
                    <div className="flex gap-2 items-center">
                        <Link href={`/motos/brand/${params.id}/model/${params.model_id}`} className={components.button.icon_small}><FiChevronLeft /></Link>
